...
def obtener_id_paciente():
    ...
    csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pacientes.csv")
...
def mostrar_instrucciones(texto):
    ...
    tk.Label(frame, text=texto, font=("Arial", 30),
             wraplength=1500, justify="center", bg=LIGHT_BG).pack(expand=True)
...
def tmt_test(lista_objetivos, titulo, alternancia=False):
    ...
    for i, val in enumerate(lista_mostrar):
        btn = tk.Button(tablero, text=str(val), width=10, height=4, 
                        command=lambda v=val: click_elemento(v), font=("Arial", 20))
...
def guardar_json():
    ...
    ruta = os.path.join(carpeta, f"{id_paciente}_tsecuencia.json")
